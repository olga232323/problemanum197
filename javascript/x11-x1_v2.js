function descodificar_x11(x11) {
    ...
    if(x11.length %2 == 0){ // verificar si la cadena de caracteres es impar o no
        var posicionCaracter = x11.length -1;
    }else{
        var posicionCaracter = x11.length -2;
    }
    var x1='';
    for (let i = 0; i < x11.length; i+=2) {
        x1+=x11[i];
    }
    for (let i = posicionCaracter; i >= 0; i-=2) {
        x1+=x11[i];
    }
    return x1;
}
